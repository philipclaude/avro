...
//Perform the LU decomposition with unit diagonal on U, where LUP is used for the inner block solves
  ...
  struct MatrixBlock_2x2_LUP
  {
    ...
    MatrixBlock_2x2_LUP(const blockType<Expr>& MatrixExpr) : MatrixFac(MatrixExpr.cast())
    {
      Decompose(MatrixFac);
    }
  ...
  template<class Expr>
  struct MatrixBlock_2x2_Decompose_LUP : public blockType< MatrixBlock_2x2_Decompose_LUP<Expr> >
  {
...
//Operator to generate a datatype to represent the LU decomposition with pivoting of a matrix expression
  ...
  Decompose_LUP(const blockType<Expr>& MatrixExpr)
  {
