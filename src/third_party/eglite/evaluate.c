...
EG_invEvaGeomLimits(const egObject *geomx, /*@null@*/ const double *limits,
                    ...
                    double *result)
{
  int            i, j, ii, iii, jjj, k, stat, per, atype, alen, cnt, jDiv, iDiv;
  double         a, b, tx, period, tol;
  double         pt[3], uvs[2], uvx[2], range[4], data[18], srange[4] = {0,0,0,0};
#ifdef LITE
  ...
  if (geom->oclass == PCURVE) {
    ...
    if (geom->mtype == BSPLINE) {
      ...
      if ((b < a) || (stat != EGADS_SUCCESS)) {
        printf(" EGADS Info: NearestOn 10x10 fails -- Seed closer stat = %d\n",
               stat);
    ...
    } else if ((geom->mtype == TOROIDAL) || (geom->mtype == BEZIER) ||
               (geom->mtype == OFFSET)) {
      for (j = 0; j < 20; j++) {
      ...
      if ((b < a) || (stat != EGADS_SUCCESS)) {
        if (stat != EGADS_EMPTY)
          printf(" EGADS Info: NearestOn 20x20 fails -- Seed closer stat = %d\n",
                 stat);
        EG_evaluateGeom(geom, uvs, data);
        param[0]  = uvs[0];
        param[1]  = uvs[1];
...
/*        printf(" Info: NEAREST Fails -- Seed point closer    stat = %d\n",
      ...
      a      = (result[0]-xyz[0])*(result[0]-xyz[0]) +
               ...
               (result[2]-xyz[2])*(result[2]-xyz[2]);
      if ((b < a) || (stat != EGADS_SUCCESS)) {
        printf(" EGADS Info: NearestOn 10x20 fails -- Seed closer stat = %d\n",
               stat);
        EG_evaluateGeom(geom, uvs, data);
        param[0]  = uvs[0];
        param[1]  = uvs[1];
      ...
      a      = (result[0]-xyz[0])*(result[0]-xyz[0]) +
               ...
               (result[2]-xyz[2])*(result[2]-xyz[2]);
      if ((b < a) || (stat != EGADS_SUCCESS)) {
        printf(" EGADS Info: NearestOn fails -- Seed closer stat = %d\n", stat);
        EG_evaluateGeom(geom, uvs, data);
        param[0]  = uvs[0];
        param[1]  = uvs[1];
        result[0] = data[0];
            ...
            /*@null@*/ double *uvx)
{
  ...
  double   tol, uvtol, nrm[3], norm[3], tng[3];
  ...
  for (i = 0; i < pface->nloops; i++) {
    ...
    for (j = 0; j < ploop->nedges; j++) {
      ...
      if (edge->mtype != DEGENERATE) {
        ...
        if (dd <= pedge->tol) {
#else
        if (dd <= BRep_Tool::Tolerance(pedge->edge)) {
#endif
          if (pt != NULL) {
            pt[0]  = eval[0];
            pt[1]  = eval[1];
