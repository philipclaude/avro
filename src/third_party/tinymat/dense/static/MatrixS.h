...
public:
  ...
  ~MatrixS() {}
  MatrixS( const typename tinymat::POD<T>::type& s );
  ...
  MatrixS& operator=( const typename tinymat::POD<T>::type& s );
...
template <int M, int N, class T>
inline
MatrixS<M,N,T>::MatrixS( const typename tinymat::POD<T>::type& s )
{
  for (int n = 0; n < M*N; n++)
    data[n] = s;
}
...
template <int M, int N, class T>
inline MatrixS<M,N,T>&
MatrixS<M,N,T>::operator=( const typename tinymat::POD<T>::type& s )
{
  for (int i = 0; i < M*N; i++)
    data[i] = s;
  return *this;
