...
template<class TV>
class SparseVector : public sparseType< SparseVector<TV>, false >
{
public:
  typedef TV Ttype;
  ...
  SparseVector( const sparseType<Expr, useRF>& r ) : m_(r.m()), values_( new TV[m_] ) { *this = r; }
...
public:
  ...
  template< class Expr > inline SparseVector& operator=( const sparseType<Expr, true>& r );
  template< class Expr > inline SparseVector& operator+=( const sparseType<Expr, true>& r );
  template< class Expr > inline SparseVector& operator-=( const sparseType<Expr, true>& r );

  template< class Expr > inline SparseVector& operator=( const sparseType<Expr, false>& r );
  template< class Expr > inline SparseVector& operator+=( const sparseType<Expr, false>& r );
  template< class Expr > inline SparseVector& operator-=( const sparseType<Expr, false>& r );
...
template<class TV>
class SparseVector< DLA::VectorD<TV> > : public sparseType< SparseVector< DLA::VectorD<TV> >, false >
{
public:
...
/* Still need to work this out
  ...
  SparseVector( const sparseType<Expr, useRF>& r ) :
    m_(v.m_), block_m_( new int[m_] ), block_i_( new int[m_] ), values_( new TV[v.value_size()] )
  {
    *this = r;
  }
