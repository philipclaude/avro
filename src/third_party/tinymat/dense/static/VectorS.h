...
public:
  ...
  ~VectorS() {}
  VectorS( const typename tinymat::POD<T>::type& s );
  ...
  VectorS& operator=( const typename tinymat::POD<T>::type& s );
...
template <int M, class T>
inline
VectorS<M,T>::VectorS( const typename tinymat::POD<T>::type& s )
{
  for (int i = 0; i < M; i++)
    data[i] = s;
}
...
template <int M, class T>
inline VectorS<M,T>&
VectorS<M,T>::operator=( const typename tinymat::POD<T>::type& s )
{
  for (int i = 0; i < M; i++)
    data[i] = s;
  return *this;
