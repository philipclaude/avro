...
#include "tinymat/dense/tools/Matrix_Util.h"
...
public:
  ...
  MatrixSymD( const typename tinymat::POD<T>::type& s , int m );
...
#undef INDEX
  ...
  MatrixSymD& operator=( const typename tinymat::POD<T>::type& s );
...
template <class T>
inline
MatrixSymD<T>::MatrixSymD( const typename tinymat::POD<T>::type& s , int m ) :
  m_(m)
{
  allocate();
  for (int n = 0; n < SIZE; n++)
...
template<class T>
inline MatrixSymD<T>&
MatrixSymD<T>::operator=( const typename tinymat::POD<T>::type& s )
{
  for (int i = 0; i < SIZE; i++)
    data[i] = s;
  return *this;
