...
UT_TEST_CASE(test1)
{
  ...
  for (index_t k=0;k<edges.size()/2;k++)
  {
    ...
    Points params(2);
    std::vector<index_t> u2v = {0}; // first index maps to the ghost vertex
    real_t u0[2] = {0,0}; // dummy coordinates for ghost
    params.create( u0 );
    for (index_t k=0;k<primitive.nodes().size();k++)
    {
      params.create( &params0[2*k] );
      params.set_entity( k , topology.points().entity(primitive.nodes()[k]) );
      u2v.push_back( primitive.nodes()[k] );
    ...
    GeometryOrientationChecker checker( topology.points() , params , u2v , entity  );
    int s = checker.signof( primitive.geometry() );
    UT_ASSERT( s>0 );

    for (index_t k=0;k<primitive.geometry().points().nb();k++)
    {
      primitive.geometry().points().set_entity( k , topology.points().entity( u2v[k] ) );
    }
  ...
  for (index_t k=0;k<topology.points().nb();k++)
  {
    ...
    // check the orientation of the original cavity
    primitive.extract_geometry(entity);
    UT_ASSERT_EQUALS( primitive.geometry().nb() , 2*n0 ); // two real triangles + 4 ghosts (one for every edge)
