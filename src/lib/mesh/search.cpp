...
ElementSearch<type>::find( real_t* x , const index_t guess )
{
  ...
  depth_ = 0;
  luna_assert( !topology_.ghost(guess) );
...
ElementSearch<type>::step( real_t* x , const index_t start )
{
  ...
  luna_assert( !topology_.ghost(start) );
  ...
  for (index_t j=0;j<nf;j++)
  {
    ...
    real_t vol = numerics::signedVolume( xj , topology_.points().dim() );
...
ElementSearch<type>::brute( real_t* x )
{
  ...
  for (index_t k=0;k<topology_.nb();k++)
  {
    ...
    for (index_t j=0;j<nf;j++)
    {
      ...
      // check the volume
      vol = numerics::signedVolume(xk,dim);
      if (vol<TOLER)
      {
        found = false;
        break;
...
ElementSearch<type>::closest( real_t* x , std::vector<real_t>& alpha ) const
{
  ...
  luna_assert( q >= topology_.points().nb_ghost() );

  // get the ball of the vertex
  std::vector<index_t> B;
  luna_assert( topology_.inverse().created() );
  ...
  for (index_t k=0;k<B.size();k++)
  {
    ...
    {
      luna_assert_msg( alphak[j] >= -1e-12 , "alpha[%lu] = %g" , j,alphak[j] );
      alpha_tot += alphak[j];
    }
    luna_assert_msg( fabs(alpha_tot-1)<1e-12 , "sum(alpha)-1 = %g" , alpha_tot-1 );
    #endif
