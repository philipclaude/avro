...
public:
  ...
  VectorD( const int m, const typename tinymat::POD<T>::type& v0 ) : VectorDView<T>(new T[m], m, v0) { MatrixDView<T>::operator=(v0); }
  ...
  VectorD& operator=( const typename tinymat::POD<T>::type& s )                              { MatrixDView<T>::operator=(s); return *this; }
