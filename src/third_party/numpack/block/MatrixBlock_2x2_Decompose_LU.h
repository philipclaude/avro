...
//Perform the LU decomposition with unit diagonal on U
  ...
  struct MatrixBlock_2x2_LU
  {
    ...
    MatrixBlock_2x2_LU(const blockType<Expr>& MatrixExpr) : MatrixFac(MatrixExpr.cast())
    {
      Decompose(MatrixFac);
    }
  ...
  template<class Expr>
  struct MatrixBlock_2x2_Decompose_LU : public blockType< MatrixBlock_2x2_Decompose_LU<Expr> >
  {
...
//Operator to generate a datatype to represent the LU decomposition without pivoting of a matrix expression
  ...
  Decompose_LU(const blockType<Expr>& MatrixExpr)
  {
  ...
  inline void
  Decompose_LU(const blockType<Expr>& MatrixExpr, MatrixBlock_2x2<M00, M01, M10, M11>& LU)
  {
    LU = MatrixExpr;
...
//  inline void
//  Decompose_LUP(const blockType<Expr>& MatrixExpr, MatrixBlock_2x2<M00, M01, M10, M11>& LU)
//  {
