...
avroWebGL.prototype.initialize = function(webglpp) {
  ...
  for (let i = 0; i < buffers.length; i++) {
    ...
    if (name.indexOf('coordinates') !== -1) {
      ...
      this.vao[idx].edges[j].visible = true;
      ...
      label = name.substr(0,idx_order-1);
      texture.name = label;

      texture.order = order;
      texture.nb_basis = (order+1)*(order+2)/2;
      this.vao[idx].triangles[j].field = texture;
      this.vao[idx].triangles[j].fields.push(texture);

      this.vao[idx].field_active = true;

      this.vao[idx].fields.add(label);
    }
    else {
  ...
  // convert each vao set of fields to a dictionary to field indices
  for (let i = 0; i < this.vao.length; i++) {

    let fields = Array.from(this.vao[i].fields);

    let field_d = {};
    for (let f = 0; f < fields.length; f++) {
      console.assert( this.vao[i].triangles.length > 0 );
      let index = -1;
      for (let j = 0; j < this.vao[i].triangles[0].fields.length; j++) {
        if (this.vao[i].triangles[0].fields[j].name == fields[f]) {
          index = j;
          break;
        }
      }
      console.assert( index >= 0 );
      field_d[fields[f]] = index;
    }
    console.log(field_d);
    this.vao[i].fields = field_d;
    this.vao[i].field_names = fields;
  }
...
avroWebGL.prototype.buildUI = function() {

  this.gui = new dat.gui.GUI({"name":"Controls"});
  let lighting = {'lighting':false};
  this.gui.add(lighting,'lighting');
  this.gui.domElement.style.zIndex = 1;

  let vis = this;
  for (let i = 0; i < this.vao.length; i++) {

    // create a plot for this vao
    let plot = this.gui.addFolder('Plot ' + i.toString());

    // get a list of all fields
    let names = ["[constant]"].concat(this.vao[i].field_names);
    plot.add({'field':"[constant]"},'field',names).onChange( function() {
      if (this.getValue() == "[constant]") {
        vis.vao[i].field_active = false;
      }
      else {
        vis.vao[i].field_active = true;
        for (let j = 0; j < vis.vao[i].triangles.length; j++) {
          vis.vao[i].triangles[j].field = vis.vao[i].triangles[j].fields[ vis.vao[i].fields[this.getValue()]];
        }
      }
      vis.draw();
    }).listen();

    let triangles = plot.add({'triangles':true},'triangles').onChange( function() {
      for (let j = 0; j < vis.vao[i].triangles.length; j++)
        vis.vao[i].triangles[j].visible = this.getValue();
      vis.draw();
    }).listen();

    let edges = plot.add({'edges':true},'edges').onChange( function() {
      for (let j = 0; j < vis.vao[i].edges.length; j++)
        vis.vao[i].edges[j].visible = this.getValue();
      vis.draw();
    }).listen();
  }
...
function VertexArrayObject() {
  ...
  this.fields = new Set(); // we will convert to a list later
}

VertexArrayObject.prototype.draw = function(gl,program) {

  if (!this.field_active) {
    gl.uniform1i( gl.getUniformLocation(program,'use_constant_color'),1);
  }
  else
    gl.uniform1i( gl.getUniformLocation(program,'use_constant_color'),-1);
  ...
  for (let i = 0; i < this.triangles.length; i++) {
    ...
    if (!this.triangles[i].visible) continue;
  ...
  for (let i = 0; i < this.edges.length; i++) {
    if (!this.edges[i].visible) continue;
