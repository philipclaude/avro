...
InverseTopology<type>::decrement( index_t bar )
{
  ...
  std::vector<index_t> b;
  for (index_t k = 0; k < nb(); k++) {
    std::unordered_set<index_t>::iterator it;
    b.clear();
    for (it = ball_[k].begin(); it != ball_[k].end(); ++it) {
      if (*it > bar) {
        b.push_back(*it -1);
      }
      else b.push_back(*it);
    ...
    ball_[k].clear();
    std::copy(b.begin(),b.end(),std::inserter(ball_[k],ball_[k].end()) );
  }
  #endif
}
...
InverseTopology<type>::update( Cavity<type>& cavity , bool delay )
{
  ...
  // update the ball of every vertex involved in the cavity insertion
  for (index_t k = 0; k < N.size(); k++) {
    ball_[N[k]].clear();
    getball(N[k],elem_[N[k]],ball_[N[k]]);
  }
}
...
InverseTopology<type>::create( index_t nb_new )
{
  for (index_t k=0;k<nb_new;k++) {
    ...
    ball_.push_back( std::unordered_set<index_t>() );
  }
}
