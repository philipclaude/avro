...
Plotter::initialize()
{
  ...
  createWindow( "avro plot" );
  GLFWwindow* window = window_["avro plot"]->window();
...
{
  if (name=="main") return window("avro plot");

  avro_assert( window_.find(name)!=window_.end() );
...
{
  avro_assert( shader_.find(name)!=shader_.end() );
