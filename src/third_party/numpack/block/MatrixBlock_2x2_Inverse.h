...
namespace BLA
{
  ...
  struct InverseSolver
  {
    ...
    static void Solve( const FactorType& MatrixFac,
                       const blockType<ExpR>& expr, const Real sgn, MatrixType& res )
    {
      res = expr;
  ...
  struct MatrixBlock_2x2_Inverse :
      public blockType< MatrixBlock_2x2_Inverse<SolverType, M00, M01, M10, M11> >
  {
    typedef MatrixBlock_2x2<M00, M01, M10, M11> MatrixType;
    typedef typename SolverType<M00, M01, M10, M11, MatrixType>::FactorType FactorType;
    ...
    inline void value(const Real sgn, blockType<RExpr>& res) const
    {
  ...
  struct MatrixSolve : public blockType< MatrixSolve<SolverType, M00, M01, M10, M11, RExpr> >
  {
    typedef MatrixBlock_2x2<M00, M01, M10, M11> MatrixType;
    typedef typename SolverType<M00, M01, M10, M11, MatrixType>::FactorType FactorType;

    template<class Expr>
    MatrixSolve( const blockType<Expr>& Matrix, const RExpr& MatrixRExpr )
      : MatrixFac(Matrix), MatrixRExpr(MatrixRExpr) {}

    //This allows for the general expression x = s*Inverse( A + B )*(a + b)
    template<class Expr>
    inline void value(const Real sgn, blockType<Expr>& res) const
    {
