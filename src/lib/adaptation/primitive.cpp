...
Primitive<type>::Primitive( Topology<type>& _topology ) :
  ...
  u_(_topology.element().parameter()?(_topology.points().dim()):(_topology.points().dim()-1)),
  geometry_topology_(u_,_topology.element().parameter()?(_topology.number()):(_topology.number()-1)),
  ...
  debug_(true)
{
  ...
  geometry_cavity_.element().set_parameter( _topology.element().parameter() );
  geometry_topology_.element().set_parameter( _topology.element().parameter() );
}
...
Primitive<type>::geometry( index_t p0 , index_t p1 ) {
  ...
  // sometimes the interior is also tagged with geometry entities
  if (this->topology_.number() == 2 && g->number() == 2) return g;
