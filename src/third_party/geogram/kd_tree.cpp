...
namespace {
    ...
    public:
        ...
        {
          avro_assert(i < nb_points_);
          avro_assert(j < nb_points_);
          return
...
namespace GEO {
    ...
    bool KdTree::stride_supported() const { return true; }
    ...
    {
        avro_assert(nb_neighbors <= nb_points());
    ...
    void KdTree::get_nearest_neighbors_recursive(
        ...
        const double* query_point, NearestNeighbors& NN ) const
    {
        avro_assert(e > b);
        	...
        	NearestNeighbors& NN ) const
    {
    	...
    	for (index_t ii=0; ii<nb; ++ii)
      {
  	    ...
  	    double sq_dist = Geom::distance2(query_point, p, dimension());
  	    local_idx[ii] = i;
    ...
    void KdTree::init_bbox_and_bbox_dist_for_traversal(
    	...
    	double& box_dist, const double* query_point ) const
      {
        // Compute distance between query point and global bounding box
        // and copy global bounding box to local variables (bbox_min, bbox_max),
        // allocated on the stack. bbox_min and bbox_max are updated during the
...
/****************************************************************************/
    ...
    index_t BalancedKdTree::build_tree()
    {
        ...
        if ( nb_points() >= (16 * MAX_LEAF_SIZE) &&
             avro::ProcessCPU::maximum_concurrent_threads() > 1 )
        {
            m0_ = 0;
            m8_ = nb_points();
