...
public:
  ...
  ALWAYS_INLINE SurrealS( const Real& v0, const Real d0[], const int n );
  ...
  template<int M, class R> friend SurrealS<M,R> operator+( const SurrealS<M,R>&, const Real& );
  template<int M, class R> friend SurrealS<M,R> operator+( const Real&, const SurrealS<M,R>& );
  template<int M, class R> friend SurrealS<M,R> operator-( const SurrealS<M,R>&, const SurrealS<M,R>& );
  template<int M, class R> friend SurrealS<M,R> operator-( const SurrealS<M,R>&, const Real& );
  template<int M, class R> friend SurrealS<M,R> operator-( const Real&, const SurrealS<M,R>& );
  template<int M, class R> friend SurrealS<M,R> operator*( const SurrealS<M,R>&, const SurrealS<M,R>& );
  ...
  template<int M, class R> friend bool operator==( const SurrealS<M,R>&, const Real& );
  template<int M, class R> friend bool operator==( const Real&, const SurrealS<M,R>& );
  template<int M, class R> friend bool operator!=( const SurrealS<M,R>&, const SurrealS<M,R>& );
  template<int M, class R> friend bool operator!=( const SurrealS<M,R>&, const Real& );
  template<int M, class R> friend bool operator!=( const Real&, const SurrealS<M,R>& );
  template<int M, class R> friend bool operator>( const SurrealS<M,R>&, const SurrealS<M,R>& );
  ...
  template<int M, class R, class S>
  friend typename std::enable_if< is_arithmetic_not_SurrealS<S>::value, SurrealS<M,R> >::type
  pow( const SurrealS<M,R>&, const S& );
  template<int M, class R, class S>
  friend typename std::enable_if< is_arithmetic_not_SurrealS<S>::value, SurrealS<M,R> >::type
  pow( const S&, const SurrealS<M,R>& );
...
template<int N, class T>
ALWAYS_INLINE
SurrealS<N,T>::SurrealS( const Real& v0, const Real d0[], const int n ) : v_(v0)
{
  SANS_ASSERT( n == N );
  for (int i = 0; i < N; i++)
    d_[i] = d0[i];
...
template<int N, class T>
ALWAYS_INLINE
SurrealS<N,T>::SurrealS( const Real& v0, const Real& d0 ) : v_(v0)
{
  for (int i = 0; i < N; i++)
    d_[i] = d0;
}
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
operator+( const SurrealS<N,T>& a, const Real& b )
{
  SurrealS<N,T> c;
  for (int i = 0; i < N; i++)
    c.d_[i] = a.d_[i];
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
operator+( const Real& a, const SurrealS<N,T>& b )
{
  SurrealS<N,T> c;
  for (int i = 0; i < N; i++)
    c.d_[i] = b.d_[i];
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T> operator/( const SurrealS<N,T>& a, const Real& b )
{
  SurrealS<N,T> c;
  Real tmp = 1./b;
  for (int i = 0; i < N; i++)
...
template<int N, class T>
ALWAYS_INLINE bool
operator==( const SurrealS<N,T>& lhs, const Real& rhs )
{
  return lhs.v_ == rhs;
}
...
template<int N, class T>
ALWAYS_INLINE bool
operator==( const Real& lhs, const SurrealS<N,T>& rhs )
{
  return lhs == rhs.v_;
}
...
SURREALS_FUNC1( tan, tan(z.v_),  Real(1)/(cos(z.v_)*cos(z.v_)) )
SURREALS_FUNC1( acos, acos(z.v_), -Real(1)/sqrt(1 - z.v_*z.v_) )
SURREALS_FUNC1( asin, asin(z.v_),  Real(1)/sqrt(1 - z.v_*z.v_) )
SURREALS_FUNC1( atan, atan(z.v_),  Real(1)/(1 + z.v_*z.v_) )

SURREALS_FUNC2( atan2, atan2(z1.v_, z2.v_),  Real(1)/(z1.v_*z1.v_ + z2.v_*z2.v_) )

// hyperbolic functions <cmath>

SURREALS_FUNC1( cosh, cosh(z.v_), sinh(z.v_) )
SURREALS_FUNC1( sinh, sinh(z.v_), cosh(z.v_) )
SURREALS_FUNC1( tanh, tanh(z.v_), Real(1)/(cosh(z.v_)*cosh(z.v_)) )

// exp and log functions <cmath>

SURREALS_FUNC1( exp, exp(z.v_), exp(z.v_) )
SURREALS_FUNC1( expm1, expm1(z.v_), exp(z.v_) )
SURREALS_FUNC1( log, log(z.v_), Real(1)/z.v_ )
SURREALS_FUNC1( log10, log10(z.v_), Real(1)/(z.v_*log(10.)) )
SURREALS_FUNC1( log1p, log1p(z.v_), Real(1)/( 1 + z.v_ ) )
...
ALWAYS_INLINE typename std::enable_if< is_arithmetic_not_SurrealS<S>::value,
                                       SurrealS<N,T> >::type
pow( const SurrealS<N,T>& a, const S& b)
{
...
ALWAYS_INLINE typename std::enable_if< is_arithmetic_not_SurrealS<S>::value,
                                       SurrealS<N,T> >::type
pow( const S& a, const SurrealS<N,T>& b)
{
  T powab = pow(a, b.v_);
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
max( const Real& a, const SurrealS<N,T>& b )
{
  if ( a > b.value() )
    return a;
  else
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
max( const SurrealS<N,T>& a, const Real& b )
{
  if ( a.value() > b )
    return a;
  else
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
min( const Real& a, const SurrealS<N,T>& b )
{
  if ( a < b.value() )
    return a;
  else
...
template<int N, class T>
ALWAYS_INLINE SurrealS<N,T>
min( const SurrealS<N,T>& a, const Real& b )
{
  if ( a.value() < b )
    return a;
  else
