...
public:
  ...
  MatrixSymS( const typename tinymat::POD<T>::type& s );
...
#undef INDEX
  ...
  MatrixSymS& operator=( const typename tinymat::POD<T>::type& s );
...
template <int M, class T>
inline
MatrixSymS<M,T>::MatrixSymS( const typename tinymat::POD<T>::type& s )
{
  for (int n = 0; n < SIZE; n++)
    data[n] = s;
}
...
template <int M, class T>
inline MatrixSymS<M,T>&
MatrixSymS<M,T>::operator=( const typename tinymat::POD<T>::type& s )
{
  for (int i = 0; i < SIZE; i++)
    data[i] = s;
  return *this;
