...
namespace GEO {
	...
	double point_coord(int index, coord_index_t coord) {
	    avro_assert(index >= 0);
	    avro_assert(index_t(index) < nb_points());
	    avro_assert(coord < dimension());
	    index_t direct_index = point_index_[index_t(index)];
	    avro_assert(direct_index < nb_points());
