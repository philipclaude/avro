...
Controller::Controller( float fov , int width , int height , float znear , float zfar ) :
  width_(width),
  height_(height),
  offleft_(0),
  offtop_(0),
...
void
Controller::mouse_down(int button, int action, int mods,int x,int y)
{
  startx_ = x;
  starty_ = y;
...
void
Controller::mouse_move(int x, int y)
{
  cursorx_ = x;
  cursory_ = y;
  ...
  //printf("modifier = %d,startx = %d, starty = %d\n",modifier,startx_,starty_);
...
void
Controller::key_down(int key)
{

}

void
Controller::mouse_wheel(double deltax ,double deltay)
{
  if (deltay > 0)
  {
