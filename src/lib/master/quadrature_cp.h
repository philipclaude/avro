 ...
 * imported into avro by Philip Caplan in 2019
 *
      ...
      double st[], int kind, double alpha, double beta, double a, double b)
{
  ...
  if (kind == 1)
  {
    avro_assert_msg(false,"KIND != 4 NOT UNIT TESTED.");

    al = 0.0;
    be = 0.0;
    if (fabs(b - a) <= temp)
      avro_assert_msg(false,"SCQF - Fatal error!\n|B - A| too small.");
    shft = (a + b)/2.0;
    slp = (b - a)/2.0;
  }
  else if (kind == 2)
  ...
  else if (kind == 9)
  {
    ...
    avro_assert_msg(false,"KIND specified is not implemented for shifts.");
...
sgqf(int nt, double aj[], double bj[], double zemu, double t[], double wts[])
{
  ...
  if (zemu <= 0.0)
    avro_assert_msg(false,"SGQF error: zemu is not positive.");
...
class_matrix(int kind, int m, double alpha, double beta, double aj[], double bj[])
{
  ...
  if (500.0*temp < fabs(pow(tgamma(temp2), 2) - pi))
    avro_assert_msg(false,"CLASS_MATRIX: Gamma function does not match machine parameters.");

  if (kind == 1)
  {
    avro_assert_msg(false,"KIND != 4 HAS NOT BEEN UNIT TESTED.\n");
  ...
  else if (kind == 8)
  {
    ...
    avro_assert_msg(false,"KIND NOT DETECTED.\n");
...
imtqlx(int n, double d[], double e[], double z[])
{
  ...
  for (lz = 1; lz <= n; lz++)
  {
    ...
    for ( ; ; )
    {
      ...
      if (itn <= j)
      {
        avro_assert_msg(false,"Iteration limit exceeded in IMTQLX.");
      }
      j = j + 1;
      g = (d[lz] - p)/(2.0*e[lz - 1]);
      r = sqrt(g*g + 1.0);
...
parchk(int kind, int m, double alpha, double beta)
{
  ...
  if (kind <= 0)
    avro_assert_msg(false,"PARCHK - Fatal error!\nKIND <= 0.\n");
...
calculateStroudQuadrature(int nDim, int order, double x[], double w[])
{
  ...
  //avro_assert_msg(order>0,"order = %d",order);
  avro_assert(order>0);
...
//  printf("nPoints= %d\n\n", nPoints);
  ...
  else if (nDim == 4)
  {
    ...
    avro_assert_msg(false,"nDim > 4 STILL NEEDS IMPLEMENTATION.");
